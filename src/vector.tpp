...
Vec<T, N>::Vec(const T (&data)[N]) { // brackets to pass ref to array, rather
                                     // than array of references
...
//   }
// }
template <typename T, size_t N> Vec<T, N>::Vec(const std::array<T, N> data) {
  for (unsigned int i = 0; i < N; i++) {
    this->data[i] = data[i];
  }
}

template <typename T, size_t N> Vec<T, N>::Vec(std::initializer_list<T> v) {
  assert(v.size() == N && "Vector initialzer list has incorrect size.");
  std::copy(v.begin(), v.end(), data.begin());
}

template <typename T, size_t N> size_t Vec<T, N>::len() { return N; }

template <typename T, size_t N> T &Vec<T, N>::operator[](size_t i) {
  assert(i < N && "Accessing outside of vector bounds.");
  return data[i];
}

template <typename T, size_t N> const T &Vec<T, N>::operator[](size_t i) const {
  assert(i < N && "Accessing outside of vector bounds.");
  return data[i];
}

template <typename T, size_t N> bool Vec<T, N>::operator==(const Vec<T, N> &v) {
  bool same = true;
...
template <typename T, size_t N> T Vec<T, N>::norm() const {
  T tot = 0;
  for (T const &i : data) {
    tot += pow(i, 2);
  }
...
template <typename T, size_t N> Vec<T, N> Vec<T, N>::unit() const {
