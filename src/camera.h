...
class Camera {
public:
  Camera(size_t width, size_t height, float fov, float nearClipping,
         float farClipping);
  ...
  void vertex_shader(const Point3h &vertex, const Matf4 &projectionMatrix,
                     const Matf4 &worldToCameraMatrix, Point3h &out);
  void texture_shader(const Mesh &mesh, ColourRGBA &out, const float &w0,
                      const float &w1, const float &w2, const Vec<float, 3> &c0,
                      const Vec<float, 3> &c1, const Vec<float, 3> &c2,
                      const float &z);
...
private:
  ...
  Point3h &compute_pixel_coordinate(const Point3h &src_pt,
  ...
  triangle_raster_bbox(const Point3h &a, const Point3h &b, const Point3h &c,
                       size_t xMaximumLimit, size_t yMaximumLimit);
};
...
public:
  ...
  virtual void render(size_t width, size_t height,
                      const std::vector<float> &imgBuffer) = 0;
};

class ppmRenderer : public Renderer {
public:
  void render(size_t width, size_t height,
              const std::vector<float> &imgBuffer) override;
};
