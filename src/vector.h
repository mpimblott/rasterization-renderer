...
template <typename T, size_t N>
Vec<T, N> operator*(typename V_traits<T>::element_type lhs,
                    const Vec<T, N> &rhs);

template <typename T, size_t N>
Vec<T, N> operator*(const Vec<T, N> &lhs,
                    typename V_traits<T>::element_type rhs);

template <typename Y, size_t M, size_t U>
Vec<Y, M> operator*(const Vec<Y, U> &lhs, const Vec<Vec<Y, M>, U> &rhs);

template <typename Y, size_t M, size_t U>
Vec<Vec<Y, 1>, M> operator*(const Vec<Vec<Y, U>, M> &lhs,
                            const Vec<Vec<Y, 1>, U> &rhs);

template <typename Y, size_t M, size_t U, size_t P>
Vec<Vec<Y, P>, M> operator*(const Vec<Vec<Y, U>, M> &lhs,
                            const Vec<Vec<Y, P>, U> &rhs);
...
template <typename Y, size_t M, size_t U>
Vec<Vec<Y, U>, M> operator/(const Vec<Vec<Y, U>, M> &lhs,
                            typename V_traits<Y>::element_type rhs);
...
template <typename T> T det(const Vec<Vec<T, 2>, 2> &m);

template <typename T> T det(const Vec<Vec<T, 3>, 3> &m);

template <typename T, size_t N> T det(const Vec<Vec<T, N>, N> &m);

template <typename T, size_t N> Vec<Vec<T, 1>, N> transpose(const Vec<T, N> &v);

template <typename T, size_t N> Vec<T, N> transpose(const Vec<Vec<T, 1>, N> &m);
...
template <typename T, size_t N> Vec<T, N> &ZeroVec(Vec<T, N> &v);
...
public:
  ...
  friend Vec<T, N> operator*
      <>(typename V_traits<T>::element_type lhs, const Vec<T, N> &rhs);
  friend Vec<T, N> operator*
      <>(const Vec<T, N> &lhs, typename V_traits<T>::element_type rhs);
  friend Vec<T, N> operator/
      <>(const Vec<T, N> &lhs, typename V_traits<T>::element_type rhs);
  template <typename Y, size_t M, size_t U>
  friend Vec<Y, M> operator*(const Vec<Y, U> &lhs,
                             const Vec<Vec<Y, M>, U> &rhs);
  template <typename Y, size_t M, size_t U>
  friend Vec<Vec<Y, 1>, M> operator*(const Vec<Vec<Y, U>, M> &lhs,
                                     const Vec<Vec<Y, 1>, U> &rhs);
  template <typename Y, size_t M, size_t U, size_t P>
  friend Vec<Vec<Y, P>, M> operator*(const Vec<Vec<Y, U>, M> &lhs,
                                     const Vec<Vec<Y, P>, U> &rhs);
  template <typename Y, size_t M, size_t U>
  friend Vec<Vec<Y, U>, M> operator/(const Vec<Vec<Y, U>, M> &lhs,
                                     typename V_traits<Y>::element_type rhs);
  T norm() const;
  Vec<T, N> unit() const;
