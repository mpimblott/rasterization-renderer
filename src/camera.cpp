...
Camera::Camera(size_t width, size_t height, float fov, float nearClipping,
               float farClipping)
    : pixelWidth(width), pixelHeight(height), fov(fov),
      nearClippingDistance(nearClipping), farClippingDistance(farClipping) {
  init();
}
...
void Camera::recompute_projection_mat() {
  ...
  projectionMat = {{s, 0, 0, 0},
                   {0, s, 0, 0},
                   {0, 0,
                    (-1 * farClippingDistance) /
                        (farClippingDistance - nearClippingDistance),
                    -1},
                   {0, 0,
                    (-1 * farClippingDistance * nearClippingDistance) /
                        (farClippingDistance - nearClippingDistance),
                    0}};
}
...
*/
Point3h &Camera::screen_to_raster(const Point3h &src_pt,
                                  Point3h &dst_pt) const {
 ...
 */
Point3h &Camera::compute_pixel_coordinate(const Point3h &src_pt,
                                          Point3h &dst_pt) const {
...
std::vector<float> Camera::build_img_buffer(const Mesh &mesh) {
  ...
  std::vector<float> depthBuffer(pixelWidth * pixelHeight,
  ...
  for (size_t f = 0; f < mesh.get_n_faces(); f++) {
    ...
    size_t ymax;
    size_t ymin;
    std::tie(xmax, xmin, ymax, ymin) =
        triangle_raster_bbox(p0, p1, p2, pixelWidth, pixelHeight);
    ...
    for (size_t y = ymin; y < ymax; y++) {
      for (size_t x = xmin; x < xmax; x++) {
        ...
        overlaps &=
            (w0 == 0 ? ((edge0.y() == 0 && edge0.x() > 0) || edge0.y() > 0)
                     : (w0 > 0));
        overlaps &=
            (w1 == 0 ? ((edge1.y() == 0 && edge1.x() > 0) || edge1.y() > 0)
                     : (w1 > 0));
        overlaps &=
            (w2 == 0 ? ((edge2.y() == 0 && edge2.x() > 0) || edge2.y() > 0)
                     : (w2 > 0));
        ...
        if (overlaps) {
          ...
          float z =
              1 / (w0 * p0y_inverted + w1 * p1y_inverted + w2 * p2y_inverted);
...
float Camera::pineda_edge(const Point3h &p, const Point3h &p0,
                          const Point3h &p1) {
  return (p.x() - p0.x()) * (p1.y() - p0.y()) -
         (p.y() - p0.y()) * (p1.x() - p0.x());
}

inline std::tuple<size_t, size_t, size_t, size_t>
Camera::triangle_raster_bbox(const Point3h &a, const Point3h &b,
...
void Camera::vertex_shader(const Point3h &vertex, const Matf4 &projectionMatrix,
  ...
  out.x() = std::min((uint32_t)(pixelWidth - 1),
                     (uint32_t)((out.x() + 1) * 0.5 * pixelWidth));
  out.y() = std::min((uint32_t)(pixelHeight - 1),
                     (uint32_t)((1 - (out.y() + 1) * 0.5) * pixelHeight));
}

void Camera::texture_shader(const Mesh &mesh, ColourRGBA &out, const float &w0,
                            const float &w1, const float &w2,
                            const Vec<float, 3> &c0, const Vec<float, 3> &c1,
                            const Vec<float, 3> &c2, const float &z) {
...
void ppmRenderer::render(size_t width, size_t height,
                         const std::vector<float> &imgBuffer) {
