...
const size_t &Mesh::get_vertex_order_idx(size_t i) const {
  assert(i < (*vertexOrderingIndices).size() &&
         "Attempt to access mesh ordering index out of bounds.");
...
const Point3h &Mesh::get_vertex(size_t i) const {
  assert(i < (*vertices).size() &&
         "Attempt to access mesh vertex out of bounds.");
  return (*vertices)[i];
}
...
Mesh::Mesh(size_t totalNonUniqueVertices,
