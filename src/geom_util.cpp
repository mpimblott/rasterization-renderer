...
shared_ptr<Mesh> loadGeoFile(const char *file) {
  ...
  try {
    ...
    std::cerr << "Number of vertices (incl repeats): " << vertsIndexArraySize
              << std::endl;
    // stores the vertex ordering data
    unique_ptr<std::vector<size_t>> vertexOrderingIndices =
        make_unique<std::vector<size_t>>(vertsIndexArraySize);
    ...
    for (uint32_t i = 0; i < vertsIndexArraySize; i++) {
      ...
      if ((*vertexOrderingIndices)[i] > numUniqueVertices)
        numUniqueVertices =
            (*vertexOrderingIndices)[i]; // track the largest vertex index
    ...
    unique_ptr<std::vector<Point3h>> vertices =
        make_unique<std::vector<Point3h>>(numUniqueVertices);
    ...
    unique_ptr<std::vector<Point3h>> normals =
        make_unique<std::vector<Point3h>>(vertsIndexArraySize);
    for (uint32_t i = 0; i < vertsIndexArraySize; i++) {
    ...
    for (uint32_t i = 0; i < vertsIndexArraySize; i++) {
      ss >> (*vertexColours)[i][0] >> (*vertexColours)[i][1] >>
          (*vertexColours)[i][2];
      std::cerr << (*vertexColours)[i] << std::endl;
    }

    shared_ptr<Mesh> mesh_out = make_shared<Mesh>(
        Mesh(vertsIndexArraySize, std::move(vertexOrderingIndices),
